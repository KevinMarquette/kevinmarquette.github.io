...
An exception is a terminating error. A thrown error will either be caught or it will terminate the current execution. By default, a non-terminating error is generated by `Write-Error` and it adds an error to the output stream without throwing an exception.
...
You can even directly call `-ErrorAction Stop` on Write-Error to make it a terminating error.

    Write-Error -Message "Huston, we have a problem." -ErrorAction Stop
    
Thank you to Lee Daily for reminding me of that one.
